...
void Renderer3D::render_v2(World* world) {
    ...
    for(unsigned i=0; i<world->lights.size(); i++) {
        point_light& light = world->lights[i];
...
void Renderer3D::draw_textured_triangle(render_point v[3], const material& mtl, const arma::vec& normal) {
    ...
    bresenham3d(v[1], v[1], mtl, normal);
    ...
    for(float y = middle_bound; y <= top_bound; y++) {
        ...
        for(float x = left; x <= right; x++) {
            ...
            if((z < pixBuf.z_buf[i]) && (z >= near)) {
                ...
                if(v < 0)v = 0;
                if(v > 1)v = 1;
                pixBuf.buf[i] = calc_color(ix, iy, z, u, v, normal, mtl);//0xff777777;//
                pixBuf.z_buf[i] = z;
            }
        }
    ...
    for(float y = middle_bound; y >= bottom_bound; y--) {
        ...
        for(float x = left; x <= right; x++) {
            ...
            if((z < pixBuf.z_buf[i]) && (z >= near)) {
                ...
                if(v < 0)v = 0;
                if(v > 1)v = 1;
                pixBuf.buf[i] = calc_color(ix, iy, z, u, v, normal, mtl);//0xff333333;//
                pixBuf.z_buf[i] = z;
            }
        }
    ...
    for(int i = 1; i <= dx; i++) {
        ...
        if(y >= 0 && y < height && x >= 0 && x < width) {
            if(pixBuf.z_buf[index] > z) {
                ...
                if(u > 1) u = 1;
                if(v > 1) v = 1;
                pixBuf.buf[index] = calc_color(x, y, z, u, v, normal, mtl);//0xff999999;//
